...
# Solver is the main function used to delete c verticies and k edges
# to maximize the shortest path from node zero to teh target node given a graph.
def solve(G):
    """
    Args:
        G: networkx.Graph
    ...
    k = 0
    s = 0
    removedC = []  #list of removed verticies
    removedK = []  #list of removed edges
    t = numNodes - 1 #target Node
    c, k = numNodesCalc(numNodes)

    ### Algorithm 1 ###
    # This algorithm finds the shortest path in the graph and iterates through
    # removing the edges and verticies and finds which edge or vertex when removed,
    # maximizes teh shortest path.
    # This algorithm is run on all inputs small, medium, and large.
    ...
    ### Algorithm 2 ###
    # This algorithm finds the shortest path in the graph and iterates through
    # removing the verticies first and finds which verticies, when deleted, maximizes shortest path
    # Then it repeats the same procedure on edges.
    # This algorithm is run on all inputs small, medium, and large.
    ...
    ### Algorithm 3 ###
    # This algorithm is brute force and goes through every vertx in the graph and finds which one,
    # when removed has teh greatest impact on the smallest path.
    # This algorithm is only run on medium size inputs
    c, k = numNodesCalc(numNodes)
    removedC3 = []
    removedK3 = []
    GTriplePrime = G.copy()
    ...
    ### Algorithm 4 ###
    # This algorithm is the same as algorithm 1, but instead of using the heuristic of shortest path,
    # it uses teh heuristic of betweenness_centrality.
    # This algorithm is run on all size inputs, small, medium, and large.
    ...
    ### Algorithm 5 ###
    # This algorithm uses recursion to see when deleting which sets of
    # verticies and edges results in the maximum shortest path.
    # This algorithm is run on all size inputs, small, medium, and large.
    # However, the algorithm is altered for medium and large inputs, by reducing the branching factor.
    ...
    (removedC5, removedK5, max_path) = analyze_graph(GPentaPrime, t, c, k, max_branching_factor)
    ### End Algorithm 5 ###


    # the rest of this function runs the input on all algorithm and finds which algorithm
    # is most optimal for that input and returns the result.
...
# calculates the c (max verticies that can be deleted) and k (max edges that can be deleted) values for each input
...
# calculates the vertex list based on the heuristic of etweenness_centrality.
...
# calculates the edge list based on the heuristic of etweenness_centrality.
...
# the recursive part of algorithm 5 that explores multiple local maxima instead of one.
def analyze_graph(G, t, c, k, max_branching_factor):
    s = 0
    if c == 0 and k == 0:
    ...
    if len(options) > 1:
        branching_factor = random.randrange(1, min(max_branching_factor, len(options)))
    elif len(options) == 1:
        branching_factor = 1
    else:
        branching_factor = 0
    ...
    for _ in range(branching_factor):
        ...
        if type(item_to_del) is int:
            GPrime.remove_node(item_to_del)
            (new_removed_c, new_removed_k, path_length) = analyze_graph(GPrime, t, c - 1, k, max_branching_factor)
            ...
            GPrime.remove_edge(*item_to_del)
            (new_removed_c, new_removed_k, path_length) = analyze_graph(GPrime, t, c, k - 1, max_branching_factor)
            if path_length > max_path:
                max_path = path_length
                new_c = new_removed_c
